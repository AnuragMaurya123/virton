...
export default function AdminPlane() {
  const [tab, setTab] = useState<string | "">("Dashboard"); // State to track the selected tab

  // Toggle between tabs when a sidebar option is clicked
    ...
    <div className="bg-blue-950">
      <div className="container flex mx-auto py-9">
            ...
            <div className="flex flex-col items-start gap-4 w-full">
              {slidebarOption.map((option, i) => (
                <div key={i} className="border-t border-b py-2 w-full">
                  <div
                    className={`${tab === option.tab ? "bg-white text-blue-950" : "bg-none text-white"} py-2 w-full text-xl font-bold p-2`}
                    onClick={() => toggleSidebar(option.tab)} // Change the selected tab
                  >
                    {option.tab}
                  </div>
                </div>
        ...
        <div className="flex flex-1 items-start justify-start bg-white py-20 px-10 rounded-3xl">
          <Plane tab={tab} /> {/* Render the selected tab content */}
        </div>
      </div>
    </div>
  );
